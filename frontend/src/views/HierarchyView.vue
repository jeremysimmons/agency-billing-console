...
const maxDepth = computed(() => Math.max(0, treeMaxDepth(tree.value) - 1))

function applyExpandLevel(level: number) {
  const nodes = tree.value ?? []
  expandedIds.value = new Set(collectIdsToDepth(nodes, level))
}

watch(
  () => tree.value,
  (nodes) => {
    if (expandLevel.value > maxDepth.value) expandLevel.value = maxDepth.value
...
function expandLevelMinus() {
  if (expandLevel.value <= 0) return
  expandLevel.value -= 1
  applyExpandLevel(expandLevel.value)
}

function expandAll() {
  expandLevel.value = maxDepth.value
  expandedIds.value = new Set(collectAllExpandableIds(tree.value ?? []))
}
...
<template>
  <section data-testid="hierarchy-view">
    ...
    <p class="hint">Space → folder → list. Names refresh when you sync. Arrow expands; row selects.</p>
